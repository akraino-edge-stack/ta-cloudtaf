...
def create_helm_packages(client, remote_dir):
    ...
    for chart in helm_charts:
        ...
        stdout = execute_command(client, command)
        helm_package = stdout.split(cloudadmin['username'] + '/')[1].rstrip()
        print helm_package
        command = "curl -sS -XPOST -H 'Content-Type: application/gzip' --data-binary @{} {}/charts/{}".format(helm_package, chart_repo, helm_package)
        execute_command(client, command)
