...
def test_kubernetes_object_quality(kube_object, expected_result, filter=".*", timeout=30, delay=0):
    ...
    if not res:
        ...
        ex.execute_unix_command("kubectl describe " + kube_object['obj_type'] + " " + kube_object['obj_name'] + " -n " + kube_object['namespace'])
...
def get_helm_chart_content(chart_name):
    ...
    return ex.execute_unix_command("ls /tmp/" + chart_name.split('/')[1] + "/templates | awk -F . '{print $1}'").split('\r\n')
...
def get_cpupools():
    ...
    cmap_str = ex.execute_unix_command("kubectl get configmap -n kube-system {cm} -o yaml".format(cm=cpu_pooling_cm_name))
                ...
                if worker_yaml['pools']:
                    for pool in worker_yaml['pools']:
                        ...
                        for sub_list in pool_str.split(','):
                            pool_list = pool_list + ([int(sub_list)] if '-' not in sub_list else range(int(sub_list.split('-')[0]), int(sub_list.split('-')[1]) + 1))
...
def get_cpu_allowed_list_from_pod(pod_name):
    ...
    for cpu in result.split(':')[1].split(','):
        pool_list = pool_list + ([int(cpu)] if '-' not in cpu else range(int(cpu.split('-')[0]), int(cpu.split('-')[1]) + 1))
    return pool_list
...
def determine_accurate_running_time_of_obj(object_type, object_name):
    hours = mins = secs = 0
    cmd = "kubectl get {obj_type} --all-namespaces --no-headers=true | grep {obj_name} | awk '{{print $NF}}'".format(obj_type=object_type, obj_name=object_name)
