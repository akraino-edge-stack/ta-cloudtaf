...
@robot_log
def gather_logs_from_remote(command, logfile_name, local_path, host, user={}): # pylint: disable=dangerous-default-value
    if not user:
...
def helm_list(release_name, add_check_arg=''):
    ...
    stdout = ex.execute_unix_command(command, fail_on_non_zero_rc=False)
    return stdout.strip()
...
def test_kubernetes_object_quality(kube_object, expected_result, grep_filter=".*", timeout=30, delay=0):
    tester_command = "kubectl get " + kube_object['obj_type'] + " --all-namespaces | grep -w " + \
                     kube_object['obj_name'] + " | grep -E '" + grep_filter + "' | wc -l"
    ...
    if not res:
        ...
        raise Exception("Not " + kube_object['obj_count'] + " " + kube_object['obj_type'] + " " +
                        kube_object['obj_name'] + " is in expected (" + grep_filter + ") state:" + res)
    logger.console(kube_object['obj_count'] + " " + kube_object['obj_type'] + " " + kube_object['obj_name'] +
                   " is in expected (" + grep_filter + ") state.")
...
def wait_if_pressure(timeout=pressure_default_timeout):
    wait_until = datetime.now() + timedelta(seconds=timeout)
    command = r"kubectl get nodes -o json | jq '.items[] | \"\(.metadata.name) \(.spec.taints)\"'"
