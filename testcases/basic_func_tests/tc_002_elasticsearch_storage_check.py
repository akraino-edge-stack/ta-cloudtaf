...
@Robot_log
def elasticsearch_get_field(field, timestamp):
    data = '{ "size": 1, ' \
           '"query": { ' \
                '"exists": { "field": "' + field + '" } }, ' \
                '"sort" : [ {"@timestamp" : {"order" : "desc"}} ] }'
    header = "Content-Type: application/json"
    es_index = "{}-{}".format(ES_IDX_PREFIX, timestamp)
...
def elasticsearch_parse_field(msg, field):
    if 'hits' not in msg:
        timestamp = common_utils.determine_accurate_running_time_of_obj('daemonset', 'fluentd').strftime('%Y.%m.%d')
        msg = elasticsearch_get_field(field, timestamp)
        if 'hits' not in msg:
...
def step1_get_elasticsearch_kubernetes_data():
    ...
    resp = elasticsearch_get_field(field, timestamp)
