...
# "2017-10-17 13:03:14 +0000 [info]: plugin/in_tail.rb:586:initialize: following tail of /var/log/containers/kube-proxy-172.24.16.104_kube-system_kube-proxy-81ea7d0e0fcfd372ac3cc2a7f980dc7761ede68566b1ef30663cbb1e46307e62.log"
...
def fluentd_logging_followup_check(nodes,followedPods):
    for key in nodes:
        ...
        if fluentd != None:
           for pod in pods:
               command = "kubectl -n kube-system logs " + fluentd + "|awk '/starting fluentd/{p=1;next}/restarting/{exit} p'|grep -c 'following.*'" + pod
               logger.info(command)
