...
def step1_shared_passed():
    ...
    try:
        common_utils.helm_install(chart_name="default/cpu-pooling-shared1", release_name="cpu-pooling",
                                  values="registry_url={url},pool_req={cpu_req},nodename={node_name}".format(url=reg, cpu_req=cpu_request, node_name=nodename))
...
def step2_shared_fail():
    try:
        common_utils.helm_install(chart_name="default/cpu-pooling-shared1", release_name="cpu-pooling",
                                  values="registry_url={reg_url},pool_req={cpus},nodename={node_name}".format(reg_url=reg, cpus=(max_shared_pool_size*1000)+100, node_name=nodename))
        common_utils.test_kubernetes_object_quality(kube_object=cpu_pooling_pod4,
                                                    expected_result="1",
                                                    filter='(Pending)\s*[0]',
                                                    timeout=90,
                                                    delay=3)
        ex.execute_unix_command('kubectl describe pod {podname} | grep "{check_str}"'.format(podname=cpu_pooling_pod4['obj_name'],
