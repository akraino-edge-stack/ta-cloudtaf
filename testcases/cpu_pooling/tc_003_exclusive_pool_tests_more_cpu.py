...
def step1_with_two_process():
    try:
        common_utils.helm_install(chart_name="default/cpu-pooling-exclusive3", release_name="cpu-pooling", values="registry_url=" + reg + ",nodename=" + nodename)
        ...
        if proc1_cpu not in exclusive_cpus:
            raise Exception('{pod}: Proc1 running on non exclusive cpu core {cpu}!'.format(pod=cpu_pooling_pod3['obj_name'], cpu=proc1_cpu))
        if proc2_cpu not in exclusive_cpus:
            raise Exception('{pod}: Proc2 running on non exclusive cpu core {cpu}!'.format(pod=cpu_pooling_pod3['obj_name'], cpu=proc2_cpu))
        if proc1_cpu == proc2_cpu:
            raise Exception('{pod}: Two processes use same cpu core: {cpu}!'.format(pod=cpu_pooling_pod3['obj_name'], cpu=proc2_cpu))
    finally:
...
def get_cpu_core_of_processes(pod_name, command):
    ...
    proc_ids = ex.execute_unix_command("kubectl exec `kubectl get pod | grep {0} | awk '{{print $1}}'` -- {1}".format(pod_name, bash_command))
