...
def step3_more_replicas_than_cpus():
    ...
    try:
        common_utils.helm_install(chart_name="default/cpu-pooling-exclusive2", release_name="cpu-pooling",
                                  values="registry_url={reg_url},nodename={node_name},replicas={cpus}".format(reg_url=reg,
                                                                                                              cpus=num_of_replicas+1,
                                                                                                              node_name=nodename))
...
def step4_request_more_than_cpus():
    ...
    try:
        common_utils.helm_install(chart_name="default/cpu-pooling-exclusive2", release_name="cpu-pooling",
                                  values="registry_url={reg_url},nodename={node_name},proc_req={cpus},pool_req={cpus}".format(reg_url=reg,
                                                                                                                              cpus=max_exclusive_pool_size+1,
                                                                                                                              node_name=nodename))
        common_utils.test_kubernetes_object_quality(kube_object=cpu_pooling_pod2,
                                                    expected_result="1",
                                                    filter='(Pending)\s*[0]',
...
def step5_less_cpu_annotation_than_request():
    ...
    try:
        common_utils.helm_install(chart_name="default/cpu-pooling-exclusive2", release_name="cpu-pooling",
                                  values="registry_url={url},nodename={node_name},proc_req={proc},pool_req={req}".format(url=reg,
                                                                                                                         proc=annotation_cpu,
                                                                                                                         req=request_cpu,
                                                                                                                         node_name=nodename))
        common_utils.check_kubernetes_object(kube_object=cpu_pooling_pod2,
        ...
        error = 'Exclusive CPU requests {req} do not match to annotation {proc}'.format(req=request_cpu, proc=annotation_cpu)
...
def get_cpu_core_of_process(pod_name, command):
    ...
    proc_id = ex.execute_unix_command("kubectl exec `kubectl get pod | grep {0} | awk '{{print $1}}'` -- {1}".format(pod_name, bash_command))
