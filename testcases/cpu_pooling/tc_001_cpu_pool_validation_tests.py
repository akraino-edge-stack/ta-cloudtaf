...
def step1_check_default_pool_cpu_node_capacity():
    ...
    try:
        common_utils.helm_install(chart_name="default/cpu-pooling-default1", release_name="cpu-pooling",
                                  values="registry_url={reg_url},nodename={node_name},cpu_request={cpu},cpu_limit={cpu}".format(reg_url=reg,
                                                                                                                                node_name=nodename,
                                                                                                                                cpu=cpu_request))
...
def step2_exclusive_and_shared():
    try:
        ...
        if not common_utils.allowed_cpus_is_in_cpu_pool(allowed_cpu_for_pod, requested_cpupool):
            raise Exception('{pod} not allocate CPUs from {req_pool} pool!'.format(pod=cpu_pooling_pod6['obj_name'], req_pool=requested_cpupool))
    finally:
        common_utils.helm_delete("cpu-pooling")
...
def step8_pod_use_default_pool_burstable():
    ...
    try:
        common_utils.helm_install(chart_name="default/cpu-pooling-default1", release_name="cpu-pooling",
                                  values="registry_url={reg_url},nodename={node_name},mem_request={mem},cpu_request={cpu}".format(reg_url=reg,
                                                                                                                                  node_name=nodename,
                                                                                                                                  mem=memory_request,
                                                                                                                                  cpu=cpu_request))
        common_utils.test_kubernetes_object_quality(kube_object=cpu_pooling_pod7,
...
def step9_1_exclusive_1_shared():
    try:
        common_utils.helm_install(chart_name="default/cpu-pooling-mix1", release_name="cpu-pooling",
                                  values="registry_url={reg_url},nodename={node_name}".format(reg_url=reg, node_name=nodename))
...
def step10_cpu_allowed_list_set_after_test_pod_deployed():
    ...
    try:
        cpu_pooling_setter["obj_count"] = ex.execute_unix_command("kubectl get pod --all-namespaces | grep setter | wc -l")
        ...
        common_utils.helm_install(chart_name="default/cpu-pooling-exclusive1", release_name="cpu-pooling", values="registry_url=" + reg + ",nodename=" + nodename)
        ...
        if cpu_setter_deleted:
            if setter_count != "0":
                ...
                ex.execute_unix_command("for i in `{search}`; do {delete} $i; done".format(search=search_cmd, delete=del_cmd))
                common_utils.check_kubernetes_object(kube_object=cpu_pooling_setter,
                                                     tester_function=common_utils.test_kubernetes_object_not_available,
                                                     timeout=90)
            ex.execute_unix_command("kubectl create -f setter.yaml")
...
def check_qos_of_pod(podname, qos_type):
    command = "kubectl describe pod `kubectl get pod | grep {0} | awk '{{print $1}}'` | grep 'QoS Class:'".format(podname)
    result = ex.execute_unix_command(command)
    if qos_type not in result:
        raise Exception("{pod} QoS should be {qos}, instead of {result}!".format(pod=podname, qos=qos_type, result=result))


@Robot_log
def get_node_cpu_capacity(node_name):
    command = "kubectl describe node `kubectl get no -L=nodename | grep {nodename} | awk '{{print $1}}'`".format(nodename=node_name)
    result = ex.execute_unix_command(command)
