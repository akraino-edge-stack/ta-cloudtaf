...
Testing the service
    ...
    ${command}=    set variable    curl http://${ip_address}:32145
    ${out}=    ssh.Execute Command    ${command}    controller-1
    log    ${out}
    Should contain    ${out}    ${search}
...
Test to create pod
    Creating pod
Test to create kubernetes service with type NodePort
    Creating kubernetes service with type NodePort
Test to get the Node IP of the pod
    Getting the Node IP of the pod
Verifying the service 
    Testing the service
Test to delete the pod
    Delete the pod
Test to delete the service
    Delete the service
