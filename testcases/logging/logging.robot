...
Testing Elasticsearch Cluster Statistics
    ${expected_out}=     set variable    elasticsearch-logging.kube-system.svc.rec.io left intact
    ${command}=    set variable    curl -k -v --user elastic:changeme 'elasticsearch-logging.kube-system.svc.rec.io:9200/_cluster/stats?human&pretty'
    ${stdout}    ${stderr}=    Execute Command    ${command}    controller-1
    Should Contain    ${stderr}    ${expected_out}
