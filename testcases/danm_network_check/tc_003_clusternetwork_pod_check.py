...
def tc_003_clusternetwork_pod_check():
    """
        ...
        danmnet_pods11: pod attached to d_test-net30 with static ip, d_test-net8 with none ip, none existing danmnet(error)
        danmnet_pods12: pod attached to d_test-net30 with static ip, d_test-net8 with dynamic ip, d_test-net25 with none ip
        danmnet_pods13: pod attached to d_test-net8 with static ip, d_test-net24(flannel) with dynamic ip, none existing danmnet(error)
...
def step2():
    # Install danmnet_pods2: ips already used from allocation pool -> 3 pods in containercreating state, check remaining assigned ips in allocation pool
...
def step4():
    #Check danmnet_pods4 attached to test-net2 with namespace kube-system, check assigned ips, static route
    common_utils.test_kubernetes_object_quality(kube_object=danmnet_pods4,
                                                expected_result=danmnet_pods4['obj_count'],
                                                filter='(Running)\s*[0]',
                                                timeout=90,
                                                delay=10)
    pod_list = get_pod_list(danmnet_pods4)
    alloc_pool = get_alloc_pool('ks_test-net2')
    danmnet_pods4['ip_list'] = get_pod_ips(pod_list)
    check_dynamic_ips(alloc_pool, danmnet_pods4['ip_list'])
    check_static_routes(pod_list, 'ks_test-net2')
...
def get_pod_list(kube_object):
    ...
    for line in execute.execute_unix_command_as_root(command).split('\r\n'):
        pod_list[line.split(' ')[1]] = {'namespace': line.split(' ')[0], 'status': line.split(' ')[2], 'restarts': line.split(' ')[3]}
    return pod_list
...
def check_mac_address(pod_list, network):
    ...
    for pod in pod_list:
        if (pod_list[pod]['status'] == 'Running') and (pod_list[pod]['restarts'] == '0'):
            command = "kubectl exec " + pod + " -n " + pod_list[pod]['namespace'] + " ip a | grep -A 1 eth0 | grep link" \
                      " | awk '{print $2}'"
...
def check_dep_count(namespace, exp_count):
    ...
    if len(danmnet_test_deps) != exp_count:
        raise Exception("Danm endpoint count is not as expected! Got: " + str(len(danmnet_test_deps)) + ", expected: " + str(exp_count))
...
def get_alloc_value(network, dictionary, resource_type):
    command = "kubectl get " + resource_type + " " + dictionary[network]['name'] + " -o yaml | grep -w alloc | awk '{print $2}'"
