...
def step10():
    ...
    common_utils.test_kubernetes_object_quality(kube_object=danmnet_pods8,
                                                ...
                                                filter=r'(Running)\s*[0]',
                                                timeout=90)
    command = "curl danmnet-pods8-1.default.svc.rec.io:4242"
...
def step13():
    ...
    execute.execute_unix_command("sed -i 's/{{ .Values.registry_url }}/" + reg + "/g' " + "/tmp/" +
                                 danmnet_pods11['obj_name'] + "/templates/" + danmnet_pods11['obj_name'] + ".yaml")
    common_utils.get_helm_chart_content('default/' + danmnet_pods13['obj_name'])
    execute.execute_unix_command("sed -i 's/{{ .Values.registry_url }}/" + reg + "/g' " + "/tmp/" +
                                 danmnet_pods13['obj_name'] + "/templates/" + danmnet_pods13['obj_name'] + ".yaml")
