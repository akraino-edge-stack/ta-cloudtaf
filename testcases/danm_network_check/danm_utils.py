    ...
    for node, node_ip in stack_infos.get_all_nodes().items():
        ...
        for network in network_properties_dict:
            if (network_properties_dict[network]['host_if'] != ''):
                ...
                if count != '1':
                    raise Exception("host interface check Failed, interface " + network_properties_dict[network]['host_if'] +
                                    " does not exist on " + node + ": " + node_ip)
                logger.info("host interface check OK, interface " + network_properties_dict[network]['host_if'] + " exists on " + node)
            else:
...
def get_resource_count(resource_type, resource_name):
    return ex.execute_unix_command("kubectl get " + resource_type + " 2>/dev/null | grep -w " + resource_name + " | wc -l")
...
def replace_ifaces_in_fetched_chart_templates(path):
    ex.execute_unix_command("sed -i 's/{{ .Values.infra_int_if }}/"+ infra_int_if + "/g' " + path)
    ex.execute_unix_command("sed -i 's/{{ .Values.infra_ext_if }}/"+ infra_ext_if + "/g' " + path)
    ex.execute_unix_command("sed -i 's/{{ .Values.infra_storage_if }}/"+ infra_storage_if + "/g' " + path)
...
def check_dynamic_ips(alloc_pool, assigned_ips):
    for ip in assigned_ips:
        if (IPAddress(alloc_pool['start']) > IPAddress(ip)) or (IPAddress(ip) > IPAddress(alloc_pool['end'])):
            raise Exception("Dynamic ip: {} is not in allocation pool: {} - {}".format(ip, alloc_pool['start'], alloc_pool['end']))
